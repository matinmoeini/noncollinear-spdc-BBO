...
theta_fixed = 28.46 * (np.pi / 180)  # Initial theta value
...
fig, ax = plt.subplots(subplot_kw={'polar': True}, figsize=(8, 10))
ax.set_theta_offset(np.pi / 2)  # Rotate by 90 degrees (pi/2 radians)
# Create a custom colormap from white to yellow
colors = [(1, 1, 1), (1, 0, 0)]  # RGB values for white and yellow
n_bins = 100  # Number of bins
cmap_name = 'white_yellow'
...
def update(frame):
    ...
    theta_fixed += 0.02 * np.pi / 180  # Increment theta_fixed
    ...
    arrthetanoncollineartype2 = np.arcsin(no1type2 * np.sin(theta)) * 180 / np.pi
    ...
    ax.pcolormesh(arrphinoncollineartype2, arrthetanoncollineartype2, masked_intensity, shading='auto', cmap=cm, norm=norm1, alpha=0.8)
    ax.pcolormesh(arrphinoncollineartype2minus, arrthetanoncollineartype2minus, masked_intensity, shading='auto', cmap=cm2, norm=norm1, alpha=0.4)
    ax.set_title(f'theta_pump {(theta_fixed * 180 / np.pi):.2f}°' ,fontsize=20)
    ax.set_theta_offset(np.pi / 2)  # Rotate by 90 degrees (pi/2 radians)
# Set up the video writer with imageio-ffmpeg
fps = 20
filename = 'type1_noncollinear_nondegenerate.mkv'
