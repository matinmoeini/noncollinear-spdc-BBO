...
theta_fixed = 29 * (np.pi / 180)  # Initial theta value
...
def update(frame):
    ...
    ax.set_title(f'Type 1 degenerate Noncollinear Phase Matching_length\nlength {(l*1e3):.2f}mm',fontsize=16)
    ax.set_theta_offset(np.pi / 2)  # Rotate by 90 degrees (pi/2 radians)
# Set up the video writer with imageio-ffmpeg
fps = 20
filename = 'type1_noncollinear_degenerate_length.mkv'
